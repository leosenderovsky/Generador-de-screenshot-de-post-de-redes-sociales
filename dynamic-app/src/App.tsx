...
const App: React.FC = () => {
  ...
  const fetchPostFromURL = useCallback(async (url: string) => {
    if (!url) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(`Error: ${json.error || 'No se pudo obtener el post'}`)
      ...
      if (json.success && json.data) {
        ...
        setPostData(prev => ({
          ...prev,
          network: d.network || prev.network,
          ...
          mediaUrl: d.mediaUrl || prev.mediaUrl,
          likes: d.likes ?? prev.likes,
          comments: d.comments ?? prev.comments,
          retweets: d.retweets ?? prev.retweets,
          date: d.date || prev.date,
          isVideo: !!d.isVideo,
          isVerified: !!d.isVerified,
        }))
      } else {
        alert(json.error || 'No se pudo obtener el post')
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row text-gray-800 dark:text-gray-200">
      <ControlPanel
        ...
        fetchPost={fetchPostFromURL}
        fetchingPost={fetchingPost}
      />
      <PreviewCanvas ref={previewRef} background={background} postData={postData} layout={layout} theme={theme} />
    </div>
  )
}
